...
// Public Macros                ////////////////////////////////////////////////
	...
	#define MAX_BUFFER (OCAE_SAMPLE_RATE/1000)
#endif // MAX_BUFFER
...
#include <cmath>
	...
	#define STEREO_TO_MONO(x) SampleType(Math_t(std::get<0>(x) + std::get<1>(x))/OCAE_SQRT_HALF)
...
#include <type_traits>
	...
	#define PRINT(p) OCAE_TO_STR(p)
		...
		#define DO_PRAGMA(x) __pragma(x)
			...
			/// Do platform-specific pragma command
		#define DO_PRAGMA(x) _Pragma(OCAE_TO_STR(x))
	#else
			/// Do platform-specific pragma command
		#define DO_PRAGMA(x)
		...
		#define OCAE_TODO(x) DO_PRAGMA(message(__FILE__ "(" PRINT(__LINE__) ") OCAE_TODO - " OCAE_TO_STR(x)))
	#elif defined(__clang__) || defined(__GNUC__)
			/// Print the to-do message
		#define OCAE_TODO(x) DO_PRAGMA(message ("OCAE_TODO - " OCAE_TO_STR(x)))
	#else
			/// Print the to-do message
		#define OCAE_TODO(x)
	#endif // _MSC_VER || __clang__ || __GNUC__
