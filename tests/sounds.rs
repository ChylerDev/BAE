...
mod tests {
    ...
    fn test_simple_sounds() {
        let mut ss = SimpleSound::new(
            1.0,
            0.5,
            Arc::new(StandardBlock::from_generator(Noise::new())),
        );
        ss.extend_modifiers(vec![
            Arc::new(StandardBlock::from_modifier(LowPass::new(
                440.0,
                1.0,
                SAMPLE_RATE as MathT,
            ))),
            Arc::new(StandardBlock::from_modifier(HighPass::new(
                220.0,
                1.0,
                SAMPLE_RATE as MathT,
            ))),
    ...
    fn test_complex_sounds() {
        ...
        let n = cs.add_block(Arc::new(StandardBlock::from_generator(Noise::new())));
        let lp = cs.add_block(Arc::new(StandardBlock::from_modifier(LowPass::new(
            440.0,
            1.0,
            SAMPLE_RATE as MathT,
        ))));
        let hp = cs.add_block(Arc::new(StandardBlock::from_modifier(HighPass::new(
            220.0,
            1.0,
            SAMPLE_RATE as MathT,
        ))));
